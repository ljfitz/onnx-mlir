 ...
 * Validation 
 * ----------
 * /scripts/docker/build_with_docker.py --external-build --build-dir build --command "build/Ubuntu1804-Release/third-party/onnx-mlir/Release/bin/onnx-mlir --EmitONNXIR --debug --run-torch-pass ./third-party/onnx-mlir/third_party/onnx/onnx/backend/test/data/node/test_maxpool_2d_pads/model.onnx 
 * 
 * Limitations
 * -----------
...
public:
  ...
  LogicalResult matchAndRewrite(Operation *op, ArrayRef<Value> operands,
    ...
    std::vector<Value> translatepadsList        = createPadsArrayAttribute (pads, ty, loc, rewriter);
    std::vector<Value> dilationonnxList         = createArrayAttribute (dilations, ty,
                    loc, rewriter);
    std::vector<Value> kernalshapeonnxList      = createArrayAttribute (kernal_shape,
                    ty, loc, rewriter);
    std::vector<Value> stridesonnxList          = createArrayAttribute (strides, ty,
                    loc, rewriter);
