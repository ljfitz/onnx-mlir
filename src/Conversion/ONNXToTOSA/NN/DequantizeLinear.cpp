...
public:
  ...
  LogicalResult matchAndRewrite(ONNXDequantizeLinearOp op, OpAdaptor,
    ...
    // attribute. If x_scale is an array, it means it is trying to run per
    ...
    // Since tosa.sub and tosa.mul don't allow different ranks, get the value
    // from the constants, and create a constant of the same rank as the input
    // out of it in order to have a correct sub and mul.
