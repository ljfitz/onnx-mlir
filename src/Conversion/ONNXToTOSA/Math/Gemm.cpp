...
public:
  ...
  LogicalResult rewriteToTosaMatMul(ONNXGemmOp op, OpAdaptor adaptor,
    ...
    A = tosa::CreateOpAndInfer<tosa::ReshapeOp>(rewriter, op->getLoc(),
        RankedTensorType::get({-1, -1, -1}, AType.getElementType()), A,
        rewriter.getI64ArrayAttr(newShapeA))
            .getResult();
    B = tosa::CreateOpAndInfer<tosa::ReshapeOp>(rewriter, op->getLoc(),
    ...
    if (transA) {
      Value targetTensor =
          mlir::tosa::getConstTensor<int32_t>(rewriter, op, {0, 2, 1}, {3})
              .value();
      Type outputType =
          RankedTensorType::get({-1, -1, -1}, AType.getElementType());
      A = tosa::CreateOpAndInfer<tosa::TransposeOp>(
    ...
    if (transB) {
      ...
      B = tosa::CreateOpAndInfer<tosa::TransposeOp>(
    ...
    if (alpha && alpha.getValueAsDouble() != 1.) {
      alphaMulResult =
          tosa::CreateOpAndInfer<tosa::MulOp>(rewriter, loc, tosaResult,
    ...
    //(A*B) + Beta * C or (A*B) + C
    if (isCPresent) {
      addRes = tosa::CreateOpAndInfer<tosa::AddOp>(
    ...
    tosa::CreateReplaceOpAndInfer<tosa::ReshapeOp>(rewriter, op, resultType,
        addRes, rewriter.getI64ArrayAttr(resultType.getShape()));
  ...
  bool rewriteToTosaFC(ONNXGemmOp op, OpAdaptor adaptor,
    ...
    if (C.getType().isa<mlir::NoneType>()) {
      ...
      C = mlir::tosa::getConstTensor<float>(rewriter, op, elements, cformat)
              .value();
    }
