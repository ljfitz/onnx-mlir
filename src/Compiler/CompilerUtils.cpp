...
llvm::cl::opt<bool> RunTorchPass("run-torch-pass", llvm::cl::Hidden,     
...
std::string onnx_mlir::getTargetFilename(
...
static int genLLVMBitcode(const mlir::OwningOpRef<ModuleOp> &module,
  ...
  string unoptimizedBitcodePath = outputNameNoExt + ".unoptimized.bc";
  ...
  optBitcode.appendStr(getOptimizationLevelOption())
      .appendStr(onnx_mlir::getTargetTripleOption())
      .appendStr(onnx_mlir::getTargetArchOption())
...
static int compileModuleToObject(const mlir::OwningOpRef<ModuleOp> &module,
  ...
  objectNameWithExt = onnx_mlir::getTargetFilename(outputNameWithoutExt, EmitObj);
...
static int compileModuleToSharedLibrary(
  ...
  libNameWithExt = onnx_mlir::getTargetFilename(outputNameNoExt, EmitLib);
...
#define NOEXECSTACK                                                            \
  { "-z", "noexecstack" }
#endif
  std::string modelSharedLibPath = onnx_mlir::getTargetFilename(jniLibBase, EmitLib);
  ...
  std::string modelJniJarPath = onnx_mlir::getTargetFilename(outputNameNoExt, EmitJNI);
...
void addKrnlToLLVMPasses(mlir::OpPassManager &pm) {
  ...
  pm.addNestedPass<func::FuncOp>(mlir::bufferization::createBufferDeallocationPass());
...
int processInputFile(string inputFilename, mlir::MLIRContext &context,
...
static int emitOutputFiles(std::string outputNameNoExt,                         
  ...
  default: {
    ...
    std::string ouputNameWithExt =
        onnx_mlir::getTargetFilename(outputNameNoExt, emissionTarget);
...
int compileModule(mlir::OwningOpRef<ModuleOp> &module,
  ...
  setupModule(module, context, outputNameNoExt);
